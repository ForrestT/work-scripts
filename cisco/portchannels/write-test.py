...
def build_portchannel(vlans):
	...
	for line in vlans:
		...
		if 'Voice' in line:
			...
			break
	po_config = ['int po10',
				'desc to hc-sw-dist-vss-1',
				'switchport trunk encapsulation dot1q',
				'switchport trunk native vlan {N}'.format(N=7),
				'switchport trunk allowed vlan {D},{V}'.format(D=6,V=2006),
...
def build_EEM_script(primary, secondary):
	...
	if secondary == '':
		...
		sec_EEM = ['event manager applet MoveSecondary',
				...
				'action 4.0 cli command "channel-group 10 mode active"',
				'action 5.0 cli command "no shut"',
				'action 6.0 cli command "end"']
...
def output_to_testrun_file(device, po_config, pri_EEM, sec_EEM, filename):
	filename.write('\n{}\n'.format(device))
...
session = Tnet('svc_Cisco','*a4!@cdshw89%)','[k33pitHID]')
...
for switch in switches:
	...
	if args.writemode:
		...
		output_to_testrun_file(switch, po_config, pri_EEM, sec_EEM, testrun_file)
	session.logout()
